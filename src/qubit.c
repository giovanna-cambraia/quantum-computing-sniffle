...
double quibit_prob1(const Qubit *q) { return c_prob(q->beta); }
...
Vector3 qubit_to_bloch(const Qubit *q) {
    ...
    double complex a = q->beta;

    // global phase doesnt matter; factor it out using alpha's phase
